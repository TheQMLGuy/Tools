/**
 * Advanced Ellipse Analyzer - Application Controller
 */
const engine = new AdvancedEllipseEngine();
let showGrid = true, showLabels = true, showDirector = false;
let viewBounds = { minX: -5, maxX: 5, minY: -4, maxY: 4 };
...
function setupEventListeners() {
    ...
    document.getElementById('showDirector')?.addEventListener('change', function () { showDirector = this.checked; render(); });
...
function onParameterChange() {
    ...
    const a = parseFloat(document.getElementById('editA')?.value) || 3;
    const b = parseFloat(document.getElementById('editB')?.value) || 2;
...
function generateLineFromInputs() {
    const theta = parseFloat(document.getElementById('lineTheta')?.value || 0) * Math.PI / 180;
    let line;
...
function updatePropertiesPanel() {
    ...
    setText('statA', fmt(p.semiMajor));
    setText('statB', fmt(p.semiMinor));
    ...
    setText('propA', fmt(p.semiMajor));
    setText('propB', fmt(p.semiMinor));
    setText('propC', fmt(p.c));
    setText('propE', fmt(p.eccentricity));
    setText('propLR', fmt(p.latusRectum));
    setText('propArea', `π × ${fmt(p.semiMajor)} × ${fmt(p.semiMinor)} ≈ ${fmt(p.area)}`);
    ...
    setText('propDirector', `x² + y² = ${fmt(p.directorCircle.radius ** 2)}`);
}
...
function render() {
    ...
    if (showDirector) drawDirectorCircle(w, h, props);
...
function drawEllipse(w, h) {
    const points = engine.generatePlotPoints(120);
    ctx.strokeStyle = '#f59e0b';
    ctx.lineWidth = 2.5;
    ctx.beginPath();
    ctx.moveTo(toCanvasX(points[0].x, w), toCanvasY(points[0].y, h));
    for (let i = 1; i < points.length; i++) {
        ctx.lineTo(toCanvasX(points[i].x, w), toCanvasY(points[i].y, h));
    }
    ctx.stroke();
...
function drawDirectorCircle(w, h, props) {
    const { center, radius } = props.directorCircle;
    const cx = toCanvasX(center.x, w), cy = toCanvasY(center.y, h);
    const rx = (radius / (viewBounds.maxX - viewBounds.minX)) * w;
    const ry = (radius / (viewBounds.maxY - viewBounds.minY)) * h;
