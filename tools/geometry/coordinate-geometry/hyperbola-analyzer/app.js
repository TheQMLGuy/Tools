/**
 * Hyperbola Analyzer - Application Controller
 */
const engine = new HyperbolaEngine();
let currentEquation = 'x²/4 - y²/9 = 1';
let showGrid = true, showLabels = true, showAsymptotes = true;
let viewBounds = { minX: -8, maxX: 8, minY: -6, maxY: 6 };
...
function setupEventListeners() {
    ...
    document.getElementById('editEquation')?.addEventListener('change', function () {
        if (this.value.trim()) { try { engine.parseEquation(this.value.trim()); if (!engine.calculate().error) { currentEquation = this.value.trim(); viewBounds = engine.getViewBounds(3); updateEquationDisplay(); updatePropertiesPanel(); render(); } } catch (e) { } }
    });
    ...
    if (presetSelect) {
        HYPERBOLA_PRESETS.forEach(p => { const opt = document.createElement('option'); opt.value = p.equation; opt.textContent = p.name; presetSelect.appendChild(opt); });
    ...
    document.getElementById('resetView')?.addEventListener('click', () => { viewBounds = engine.getViewBounds(3); render(); });
}
...
function drawHyperbola(w, h) {
    const curves = engine.generatePlotPoints(3, 100);
    ctx.strokeStyle = '#10b981';
    ctx.lineWidth = 2.5;

    // Draw branch 1
    ctx.beginPath();
    curves.branch1.forEach((p, i) => {
        const cx = toCanvasX(p.x, w), cy = toCanvasY(p.y, h);
        if (cy > -500 && cy < h + 500) {
            i === 0 || cy < -400 || cy > h + 400 ? ctx.moveTo(cx, cy) : ctx.lineTo(cx, cy);
        }
    });
    ctx.stroke();
    ...
    curves.branch1.forEach((p, i) => {
        const cx = toCanvasX(p.x, w), cy = toCanvasY(p.y, h);
        if (cy > -300 && cy < h + 300) { i === 0 ? ctx.moveTo(cx, cy) : ctx.lineTo(cx, cy); }
    });
    ctx.stroke();
    ctx.beginPath();
    curves.branch2.forEach((p, i) => {
...
function drawAsymptotes(w, h, props) {
    ...
    props.asymptotes.forEach(a => {
        const slope = a.slope;
        const center = props.center;
        // Draw line through center with given slope
        const x1 = viewBounds.minX, y1 = center.k + slope * (x1 - center.h);
        const x2 = viewBounds.maxX, y2 = center.k + slope * (x2 - center.h);
        ctx.beginPath();
...
function updatePropertiesPanel() {
    ...
    setText('statA', formatNum(p.semiTransverseAxis));
    setText('statB', formatNum(p.semiConjugateAxis));
    setText('statEcc', formatNum(p.eccentricity));
    setText('propCenter', `(${formatNum(p.center.x)}, ${formatNum(p.center.y)})`);
    setText('propA', formatNum(p.semiTransverseAxis));
    setText('propB', formatNum(p.semiConjugateAxis));
    ...
    setText('propAsym1', p.asymptotes[0].equation);
    setText('propAsym2', p.asymptotes[1].equation);
